...
export default function BootcampPage() {
  ...
  const bootcamps = data?.data ?? []; //  API بيرجع data.data

  const [addBootcamp] = useAddBootcampMutation();
  const [isOpen, setIsOpen] = useState(false);

  //  Add Bootcamp Handler
  ...
  return (
    <div className="container mx-auto py-6">
      ...
      <DataTable<BootcampType>
        ...
        searchConfig={{
          enabled: true,
          placeholder: "ابحث باسم أو تاريخ البوتكامب...",
          searchKeys: ["name", "date"], // ✅ السيرش على name + date
        }}
        actionConfig={{
          enabled: true,
          showAdd: true,
        ...
        onDataChange={() => {}} // في حالة تعديل البيانات
      />
    </div>
  );
}
