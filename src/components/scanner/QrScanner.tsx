...
type TorchTrack = MediaStreamTrack & { applyConstraints?: (constraints: TorchConstraints) => Promise<void> };

export default function QrScanner({ onScanSuccess, onError, onClose }: ScannerProps) {
	...
	const turnOffTorchAndStop = useCallback(async () => {
		try {
			const videoEl = document.querySelector(`#${previewId} video`) as HTMLVideoElement | null;
	...
	}, [onScanSuccess, onError, requestPermission, previewId, turnOffTorchAndStop]);
			...
			<div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
				<Card className="w-full max-w-md">
					<CardContent className="p-6 text-center space-y-4">
						...
						<p className="text-gray-600">The app needs camera access to scan QR.</p>
