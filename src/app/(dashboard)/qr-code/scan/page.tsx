...
export default function ScanQrCodePage() {
    ...
    const handleScanSuccess = async (scannedData: string) => {
        ...
        try {
            ...
            const errorMessage = apiErr?.data?.msg || apiErr?.data?.message || `An error occurred.`;
    ...
    return (
        <div className="container mx-auto space-y-8">
                    ...
                    {bootcamp && (
                        <Card>
                            ...
                            <CardFooter className="grid grid-cols-2 gap-2 max-[450px]:grid-cols-1">
                                ...
                                <Link className="w-full" href={`/bootcamp/bootcamps/${bootcamp.id}`}>
            ...
            {/* Collections Section */}
            <section>
                <h2 className="text-2xl font-semibold mb-6 border-b pb-2">Collections</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {collections.map((collection, idx) => (
                        <Card key={collection.id}>
                            ...
                            <CardContent>
                                ...
                                <p>Materials: {collection.materials.map(m => m.material_name).join(', ')}</p>
                            </CardContent>
                            <CardFooter className="grid grid-cols-2 gap-2 max-[450px]:grid-cols-1">
                                <Button onClick={() => openScanner({ type: 'collection', collection })}>
                                    <QrCode className="mr-2 h-4 w-4" /> Scan
                                </Button>
                                <Link className="w-full" href={`/materials/collections/${collection.id}`}>
                                    <Button className="w-full" variant="outline">
                                        <User className="mr-2 h-4 w-4" /> 
                                        <p>Registrants</p>
                                        <ChevronRight />
