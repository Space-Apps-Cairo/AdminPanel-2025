...
import { useAddNewWorkshopMutation, useGetAllWorkshopsQuery } from '@/service/Api/workshops';
import { FieldValues } from 'react-hook-form';

export default function Workshops() {

    const {data: workshopsData, isLoading: isLoadingWorkshops, error: workshopsError} = useGetAllWorkshopsQuery();

    const [workshops, setWorkshops] = useState<Workshop[]>([]);
    const [isOpen, setIsOpen] = useState(false);

    useEffect(() => {
        if (workshopsData && workshopsData.data && !isLoadingWorkshops && !workshopsError) {
    ...
    const [addWorkshop, { isLoading: isAddingWorkshop }] = useAddNewWorkshopMutation();

    const handleAddWorkshopSubmit = async (data: FieldValues, formData?: FormData) => {
        try {
    ...
    if (workshopsError) {
        return <div className="container mx-auto py-6">
            <div className="text-red-500">Error loading workshops: {JSON.stringify(workshopsError)}</div>
        </div>;
    }
