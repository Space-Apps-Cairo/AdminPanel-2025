...
export const getBootcampDetailsFields = (
  bootcampData?: BootcampDetailsType
): Field[] => [
  {
    name: "name",
    type: "text",
...

  {
    ...
    ...(bootcampData?.total_capacity && {
      defaultValue: bootcampData.total_capacity,
    }),
    step: 1,
  },
];
...
export const bootcampDetailsColumns: ColumnDef<BootcampDetailsType>[] = [
...

  {
    ...
    cell: ({ row }) => <BootcampDetailsRowActions rowData={row.original} />,
    size: 150,
    enableHiding: false,
  },
];

function BootcampDetailsRowActions({
  rowData,
}: {
  rowData: BootcampDetailsType;
}) {
        ...
        <RowsActions
          ...
          updateMutation={(data: BootcampDetailsType) => {
            ...
            return updateBootcampDetails({
              id: rowData.id,
              data: formattedData as BootcampDetailsRequest,
            });
          }}
          ...
          onUpdateSuccess={(result) => {
            ...
            toast.success(
              result.message || "Bootcamp Details updated successfully!"
            );
          }}
          onUpdateError={(error) => {
            ...
            toast.error(
              error.data?.message ||
                "Failed to update bootcamp details. Please try again."
            );
          }}
          onDeleteSuccess={(result) => {
            console.log("Bootcamp Details deleted successfully:", result);
            toast.success(
              result.message || "Bootcamp Details deleted successfully!"
            );
          }}
          onDeleteError={(error) => {
            ...
            toast.error(
              error.data?.message ||
                "Failed to delete bootcamp details. Please try again."
            );
          }}
        />
      </div>
        ...
        <QrScanner
          ...
          onError={(msg) => {
            ...
            toast.error("Scan error", {
              description:
                msg || "An error occurred while registering the participant.",
            });
          }}
